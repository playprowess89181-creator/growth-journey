...
@php
    ...
    foreach ($languages as $code => $name) {
        ...
        $oldDescription = trim((string) old('translations.'.$code.'.description', ''));
        $initialHasTranslation[$code] = $oldTitle !== '' || $oldDescription !== '';
    }
@endphp
<div class="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 p-4 sm:p-6 lg:p-8">
    <div class="max-w-5xl mx-auto mb-8">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                ...
                <p class="mt-2 text-gray-600">Create the module structure once, then add translations per language.</p>
            </div>
    ...
    <div class="max-w-5xl mx-auto">
        <div class="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 overflow-hidden">
            <form method="POST" action="{{ route('admin.modules.store') }}" class="p-6 space-y-8">
                @csrf
                        ...
                        <div x-show="tab === '{{ $code }}'" class="grid grid-cols-1 gap-6">
                            ...
                            <div>
                                ...
                                <textarea name="translations[{{ $code }}][description]"
                                          rows="4"
                                          class="mt-1 block w-full rounded-xl border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                          @input="updateHas('{{ $code }}')">{{ old('translations.'.$code.'.description') }}</textarea>
                ...
                <div class="flex justify-end gap-3">
                    <a href="{{ route('admin.modules.index') }}" class="px-4 py-2 rounded-xl border border-gray-300 text-gray-700 hover:bg-gray-50">Cancel</a>
                    <button type="submit" class="px-4 py-2 rounded-xl bg-indigo-600 text-white hover:bg-indigo-700 shadow">Create Module</button>
                </div>
            </form>
        </div>
    </div>
