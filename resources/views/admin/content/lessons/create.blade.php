...
<div class="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 p-4 sm:p-6 lg:p-8">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="text-3xl sm:text-4xl font-bold bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 bg-clip-text text-transparent">
                    {{ $moduleTitle }} — Level {{ $levelNumber }} — New Lesson
                </h1>
                <p class="mt-2 text-gray-600">Add lesson structure and translations.</p>
            </div>
    ...
    <div class="max-w-5xl mx-auto">
        <div class="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 overflow-hidden">
            <form method="POST" action="{{ route('admin.lessons.store', [$module, $level]) }}" class="p-6 space-y-8">
                @csrf
                        ...
                        <div x-show="tab === '{{ $code }}'" class="grid grid-cols-1 gap-6">
                            ...
                            <div>
                                ...
                                <textarea data-lang="{{ $code }}"
                                          ...
                                          {{ $code === 'en' ? 'required' : '' }}>{{ old('translations.'.$code.'.content') }}</textarea>
                ...
                <div class="flex justify-end gap-3">
                    <button type="submit" class="px-4 py-2 rounded-xl bg-indigo-600 text-white hover:bg-indigo-700 shadow">Create Lesson</button>
                </div>
            </form>
        </div>
    </div>
